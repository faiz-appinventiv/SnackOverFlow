...
import {submitDetails} from '../../RegisterScreen/action';
...
export default function Signin({navigation}: any) {
  ...
  const onButtonPress = () => {
    AuthFunctions.signInWithEmailAndPassword(
      ...
      password,
      (userDetails: any) => {
        console.log('USERF', userDetails.user.uid);
        setEmail('');
        setPassword('');
        database()
          .ref(`/users/${userDetails.user.uid}`)
          .on('value', snapshot => {
            if (snapshot.val().fname) {
              dispatch(StoreUID(userDetails.user.uid));
              dispatch(submitDetails(snapshot.val()));
            }
          });
  ...
  return (
    <KeyboardAvoidingView style={styles.container}>
      ...
      <View style={styles.signupSection}>
        <Text style={styles.signupStatement}>{"Don't have an account? "}</Text>
